...
featureTransform <- function(df) {
  X <- cbind(df$RFSize, df$RFSize^2, df$BranchesAllowed, df$Depth, df$BranchesAllowed, df$Depth*df$BranchesAllowed)
  X <- scale(X, colMeans(X), apply(X,2,sd))
  X <- cbind(rep(1,nrow(X)),X)
  return(X)
}
...
for(lambda in c(0.0001,0.001,0.01,0.1,1,10,100,1000)) {
